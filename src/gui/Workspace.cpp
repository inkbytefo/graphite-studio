...
void SetupDefaultLayout(ImGuiID dockspace_id, bool force) {
    ...
    // Split right panel from the remaining
    ImGuiID dock_right;
    ImGuiID dock_center;
    ImGui::DockBuilderSplitNode(dock_remaining, ImGuiDir_Right, 0.22f, &dock_right, &dock_center);

    // Split right panel into top (Properties) and bottom (Layers)
    ...
    // Make toolbar node non-resizable tab bar hidden
